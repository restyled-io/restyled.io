...
getAdminR = do
  ...
  let
    trialMrr = sum $ map fst trialMrrs
...
fetchRepoStats timeRange = do
  ...
  reposWithActivityPrevious <-
    fetchReposWithActivity
      $ timeRangeBefore timeRange

  let
    totalRepos = length reposWithActivity
...
fetchJobStats timeRange = do
  ...
  let
    totalJobs = getSum $ jmTotal metrics
    totalJobsChanged =
      changedPercentage
